...
export const fetchLeadershipDashboard = async () => {
  const response = await api.get<LeadershipDashboardData>('/dashboard/leadership');
  return response.data;
};

export const fetchDeliveryDashboard = async (managerId: string) => {
...
export const fetchEngineerDetails = async (engineerId: string) => {
  const response = await api.get<EngineerDetailsData>(`/dashboard/engineer/${engineerId}`);
  return response.data;
};

export const chatWithCopilot = async (question: string) => {
